...
class GUIMaker(object):
    def __init__(self):
        ...
        #self.labelL = Label(self.tk, text='난이도').grid(row=6, column=0)
        #self.level = Scale(self.tk, from_= 10, to=100, orient=HORIZONTAL).grid(row = 6, column=1)
        #self.level.set(80)
        #self.level.pack()

        self.btn = Button(self.tk,text='Mapping',bg='black',fg='white',command=self.mapping).grid(row=7,column=0, columnspan=4)
